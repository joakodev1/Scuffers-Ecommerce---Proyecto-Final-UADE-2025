...
const API_BASE_URL = BASE_URL; // ya tiene /api al final

// 🔐 Detecta si un string "parece" un JWT
...
function getJwtToken() {
  ...
  const jsonKeys = ["authTokens", "auth_tokens", "auth", "user", "scuffers_auth"];
...
export default function Cart() {
  ...
  function getItemSlug(item) {
    return item.producto?.slug || item.product?.slug || item.slug || item.product_slug || "";
  }
  ...
  function getItemImageSrc(item) {
    ...
    const raw =
      ...
      p.image || p.image_hover || p.imagen || p.image_url || p.foto || "";
  ...
  async function handlePlus(item) {
    ...
    if (!slug) return;
    await addToCart(slug, 1, getItemSize(item));
  }

  async function handleMinus(item) {
    const slug = getItemSlug(item);
    if (!slug) return;
    await removeFromCart(slug, 1, getItemSize(item));
  }

  async function handleRemove(item) {
    const slug = getItemSlug(item);
    if (!slug) return;
    await removeFromCart(slug, Number(item.cantidad ?? 1), getItemSize(item));
  }

  async function handleMercadoPagoCheckout() {
    if (totalItems === 0 || items.length === 0 || checkingOut || updating) return;
    ...
    try {
      ...
      if (!token) {
        alert("No se encontró tu sesión. Volvé a iniciar sesión.");
        return;
      }

      // 1) Crear pedido
      const createRes = await fetch(`${API_BASE_URL}/checkout/create-order/`, {
        method: "POST",
        headers: {
      ...
      // 2) Confirmar envío
      const shippingRes = await fetch(`${API_BASE_URL}/checkout/confirm-shipping/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const mpRes = await fetch(
        `${API_BASE_URL}/checkout/mercadopago/preference/${orderId}/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!mpRes.ok) {
        ...
        const mpMsg = err?.mp_error?.message || err?.detail || "No se pudo iniciar el pago.";
      ...
      <div className="max-w-6xl mx-auto grid lg:grid-cols-[2fr,1fr] gap-10">
        <section>
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                {totalItems === 0
                  ...
                  : `Tenés ${totalItems} producto${totalItems !== 1 ? "s" : ""} en tu carrito.`}
              </p>
            </div>
            <Link to="/shop" className="text-sm text-sky-600 hover:text-sky-700 hover:underline">
              ← Seguir comprando
            </Link>
          </header>

          {loading && <p className="text-sm text-slate-600">Cargando carrito...</p>}
          ...
          <div className="space-y-4">
            {items.map((item, index) => {
              const idKey = item.id || `${getItemSlug(item) || "item"}-${getItemSize(item) || "no-size"}-${index}`;
              ...
              return (
                ...
                >
                  <div className="h-24 w-24 rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden">
                    {imgSrc ? (
                      <img src={imgSrc} alt={getItemName(item)} className="w-full h-full object-contain" />
                    ) : (
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">{getItemCategory(item)}</p>
                      <p className="text-sm font-medium text-slate-900">{getItemName(item)}</p>
                      {getItemSize(item) && (
                        <p className="text-xs text-slate-500 mt-0.5">
                          Talle: <span className="font-semibold">{getItemSize(item)}</span>
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col items-start gap-1 mt-1">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <div className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50">
                          <button type="button" onClick={() => handleMinus(item)} disabled={updating} className="px-2 py-1 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">−</button>
                          <span className="px-3 text-xs font-medium text-slate-900">{qty}</span>
                          <button type="button" onClick={() => handlePlus(item)} disabled={updating} className="px-2 py-1 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">+</button>
                        </div>
                      </div>
                      <button type="button" onClick={() => handleRemove(item)} disabled={updating} className="text-[11px] text-slate-400 hover:text-red-500 hover:underline disabled:opacity-50">
                        Eliminar producto
                      </button>
                    </div>
                  </div>
        ...
        <aside className="lg:mt-4">
          <div className="rounded-3xl bg-white border border-slate-200/70 shadow-md p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-4">Resumen de compra</h2>
            ...
            <motion.button
              whileHover={{ scale: updating || checkingOut || totalItems === 0 ? 1 : 1.01, y: updating || checkingOut || totalItems === 0 ? 0 : -1 }}
              whileTap={{ scale: updating || checkingOut || totalItems === 0 ? 1 : 0.98, y: 0 }}
              type="button"
              onClick={handleMercadoPagoCheckout}
              disabled={updating || checkingOut || totalItems === 0 || items.length === 0}
